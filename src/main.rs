...
fn enum_test() {
	...
	if let Message::Move{x: 2, ..} = move_msg { //names inside {} have to match the ones in the definition, to ignore the rest use .., x: 2 means check if x = 2 before continuing
